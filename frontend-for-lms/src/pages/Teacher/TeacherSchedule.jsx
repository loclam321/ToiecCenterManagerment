...
import { getTeacherWeeklySchedules } from '../../services/teacherScheduleService';
import '../student/css/StudentSchedule.css';
...
const parseTimeToMinutes = (timeStr) => {
  ...
  if (TIME_CACHE.has(timeStr)) return TIME_CACHE.get(timeStr);
...
const doesSessionCoverSlot = (session, slot) => {
  ...
  if (sessionStart === null || sessionEnd === null || slotStart === null || slotEnd === null) return false;
...
const calculateRowSpan = (session, currentIndex, totalSlots) => {
  const sessionStart = parseTimeToMinutes(session.schedule_startime);
  const sessionEnd = parseTimeToMinutes(session.schedule_endtime);
  if (sessionStart === null || sessionEnd === null) return 1;
...
function TeacherSchedule() {
  ...
  const schedulesByDay = useMemo(() => {
    ...
    weekDays.forEach((day) => {
      map.set(day.iso, { sessions: [], slotMap: new Map() });
    });
    scheduleItems.forEach((item) => {
      const dayKey = item.schedule_date;
      if (!map.has(dayKey)) map.set(dayKey, { sessions: [], slotMap: new Map() });
    ...
    weekDays.forEach(({ iso }) => {
      ...
      timeSlots.forEach((slot, slotIndex) => {
        const coveringSessions = dayData.sessions.filter((session) => doesSessionCoverSlot(session, slot));
  ...
  useEffect(() => {
    if (!teacherId) {
      toast.error('Không tìm thấy thông tin giáo viên. Vui lòng đăng nhập lại.');
      return;
    }
    ...
    const fetchSchedule = async () => {
      try {
        setLoading(true);
        const data = await getTeacherWeeklySchedules({
          teacherId,
        ...
        toast.error(error.message || 'Không thể tải lịch dạy.');
        setScheduleItems([]);
      } finally {
        setLoading(false);
      }
    ...
    fetchSchedule();
  }, [teacherId, weekStart, weekEnd, selectedCourse, selectedClass]);
  ...
  const shouldSkipCell = useCallback(
    (dayKey, slotIndex) => mergedTracker[dayKey]?.has(slotIndex) || false,
    [mergedTracker]
  );

  const markMergedCells = useCallback(
    (dayKey, slotIndex, span) => {
      if (!mergedTracker[dayKey]) mergedTracker[dayKey] = new Set();
  ...
  return (
    <div className="student-schedule-page">
      <div className="schedule-header">
        <div>
          <h4 className="schedule-title">Lịch dạy trong tuần</h4>
          ...
          <thead>
            <tr>
              ...
              {weekDays.map((day) => (
                <th
                  key={day.iso}
                  className={`day-column ${day.iso === formatDateISO(new Date()) ? 'today' : ''}`}
                >
            ...
            {timeSlots.map((slot, timeIndex) => (
              <tr key={slot.start}>
                ...
                {weekDays.map((day) => {
                  ...
                  if (shouldSkipCell(day.iso, timeIndex)) return null;
                  ...
                  return (
                    <td
                      ...
                      className={`session-cell ${statusClass}`}
                      rowSpan={rowSpan}
                    >
                      <div
                      ...
                      >
                        ...
                        <div className="session-meta">
                          <div>
                            <i className="fas fa-chalkboard-teacher" /> {teacherName}
                          </div>
                          <div>
                            <i className="fas fa-door-open" /> {roomName}
                          </div>
                        </div>
      ...
      {!loading && scheduleItems.length === 0 && (
        <div className="schedule-empty">
          ...
          <p>Không có buổi dạy nào trong khoảng thời gian này.</p>
        </div>
      )}
