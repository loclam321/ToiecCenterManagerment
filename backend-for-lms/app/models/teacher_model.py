...
class Teacher(db.Model):
    ...
    tch_avtlink = db.Column(
        db.String(255), nullable=True
    )  # Liên kết đến trang cá nhân hoặc hồ sơ
    tch_status = db.Column(
        db.String(20), nullable=True, default="active"
    ...
    def to_dict(self):
        ...
        return {
            ...
            "user_birthday": (
                self.user_birthday.strftime("%Y-%m-%d") if self.user_birthday else None
            ),
            "user_telephone": self.user_telephone,
            "tch_specialization": self.tch_specialization,
            "tch_qualification": self.tch_qualification,
            "tch_hire_date": (
                self.tch_hire_date.strftime("%Y-%m-%d") if self.tch_hire_date else None
            ),
            "tch_avtlink": self._avatar_web_path(),
            "tch_status": self.tch_status,  # Thêm trạng thái vào dict trả về
    ...
    def get_age(self):
        ...
        return (
            today.year
            - self.user_birthday.year
            - (
                (today.month, today.day)
                < (self.user_birthday.month, self.user_birthday.day)
            )
        )
    ...
    def get_years_of_service(self):
        ...
        if (today.month, today.day) < (
            self.tch_hire_date.month,
            self.tch_hire_date.day,
        ):
            years -= 1
