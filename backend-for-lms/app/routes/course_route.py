...
@course_bp.route("/page", methods=["GET"])
def get_courses():
...
def list_courses():
    try:
        ...
        courses = course_service.get_all_courses(status=status)
...
def toggle_course_status(course_id):
    try:
        ...
        return success_response(
            {"course": course.to_dict()}, "Đã đổi trạng thái khóa học"
        )
    except Exception as e:
    ...
    try:
        ...
        if not status:
            return validation_error_response(
                "Dữ liệu không hợp lệ", {"status": "status is required"}
            )
        ...
        if not updated:
            return not_found_response(
                "Không tìm thấy khóa học hoặc không thể cập nhật", "Course", course_id
            )
        return success_response(
            {"course": updated.to_dict()}, "Đã cập nhật trạng thái khóa học"
        )
    except Exception as e:
