...
def get_all_teachers():
    """Lấy danh sách tất cả giáo viên (có phân trang và tìm kiếm)"""
    try:
...
def get_senior_teachers():
    ...
    try:
        ...
        return success_response({"senior_teachers": teachers})
    except Exception as e:
        return error_response(
            f"Lỗi khi lấy danh sách giáo viên thâm niên: {str(e)}", 500
        )
...
def create_teacher():
    ...
    try:
        ...
        if not validation_result["valid"]:
            return error_response(
                "Dữ liệu không hợp lệ", 400, None, validation_result["errors"]
            )
        ...
        return success_response({"teacher": teacher}, 201)
    except Exception as e:
...
def update_teacher(user_id):
    ...
    try:
        ...
        if not teacher:
            return error_response(
                "Không tìm thấy giáo viên hoặc không thể cập nhật", 404
            )
