"""extend test model with class relations and scheduling metadata

Revision ID: c1b7e2d3f4a5
Revises: 8b0ceb2415ef
Create Date: 2025-10-17 10:15:00.000000
...
def upgrade():
    with op.batch_alter_table('tests', schema=None) as batch_op:
        ...
        batch_op.add_column(sa.Column('max_attempts', sa.Integer(), nullable=True, server_default='2'))
        ...
        batch_op.create_index('ix_tests_class_id', ['class_id'])
        batch_op.create_index('ix_tests_teacher_id', ['teacher_id'])
        batch_op.create_foreign_key('fk_tests_class_id', 'classes', ['class_id'], ['class_id'], ondelete='SET NULL', onupdate='CASCADE')
        batch_op.create_foreign_key('fk_tests_teacher_id', 'teachers', ['teacher_id'], ['user_id'], ondelete='SET NULL', onupdate='CASCADE')

    # Ensure existing rows have sensible defaults for new columns
    op.execute("UPDATE tests SET max_attempts = COALESCE(max_attempts, 2)")
    op.execute("UPDATE tests SET created_at = COALESCE(created_at, CURRENT_TIMESTAMP)")
    op.execute("UPDATE tests SET updated_at = COALESCE(updated_at, CURRENT_TIMESTAMP)")
