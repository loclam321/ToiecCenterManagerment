...
def upgrade():
    ...
    op.create_table(
        ...
        sa.Column(
            "course_name", sa.String(length=100), nullable=False
        ),  # Thêm nullable=False
        sa.Column(
            "course_description", sa.Text(), nullable=True
        ),  # Sửa từ String sang Text
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["part_id"], ["part.part_id"], onupdate="RESTRICT", ondelete="RESTRICT"
        ),
        sa.ForeignKeyConstraint(
            ["test_id"], ["tests.test_id"], onupdate="RESTRICT", ondelete="RESTRICT"
        ),
    ...
    op.create_table(
        ...
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=True),
        sa.ForeignKeyConstraint(
            ["course_id"], ["courses.course_id"], onupdate="CASCADE", ondelete="CASCADE"
        ),
        sa.PrimaryKeyConstraint("course_id"),
    )
    with op.batch_alter_table("learning_paths", schema=None) as batch_op:
        batch_op.create_index(
            batch_op.f("ix_learning_paths_lp_id"), ["lp_id"], unique=True
        )
    ...
    op.create_table(
        ...
        sa.ForeignKeyConstraint(
            ["user_id", "class_id"],
            ["enrollments.user_id", "enrollments.class_id"],
            onupdate="RESTRICT",
            ondelete="RESTRICT",
        ),
