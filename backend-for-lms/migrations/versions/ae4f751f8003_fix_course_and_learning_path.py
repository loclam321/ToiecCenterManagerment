...
def downgrade():
    ...
    with op.batch_alter_table("learning_paths", schema=None) as batch_op:
        batch_op.drop_constraint("learning_paths_ibfk_1", type_="foreignkey")
        batch_op.create_foreign_key(
            "learning_paths_ibfk_1",
            "courses",  # Sửa từ "course" thành "courses"
            ["course_id"],
            ["course_id"],
            onupdate="CASCADE",
            ondelete="CASCADE",
    ...
    with op.batch_alter_table("classes", schema=None) as batch_op:
        ...
        batch_op.create_foreign_key(
            ...
            "courses",
            ["course_id"],
            ["course_id"],  # Sửa từ "course" thành "courses"
        )

    # Comment hoặc xóa phần tạo lại bảng course cũ
    # op.create_table(
    #     "course",
    #     ...
    # )

    # Nếu cần drop bảng courses, uncomment dòng này
